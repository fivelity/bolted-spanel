...
async def get_hardware_history(
	...
	try:
		...
		stmt = select(HardwareDataRecord).where(
			HardwareDataRecord.timestamp >= start_time
		).order_by(desc(HardwareDataRecord.timestamp)).limit(1000)
		...
		for record in records:
			if record.raw_data:
				try:
					...
					logger.warning(f"Failed to parse historical data record {record.id}: {e}")
