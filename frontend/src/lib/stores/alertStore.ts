...
export const alertStore = {
	...
	removeCondition(conditionId: string) {
		alertConditions.update(conditions => conditions.filter(c => c.id !== conditionId));
		this.saveConditions();
	},
	...
	saveConditions() {
		if (typeof window !== 'undefined') {
			localStorage.setItem('sensecanvas-alerts', JSON.stringify(get(alertConditions)));
		}
	},

	saveHistory() {
		if (typeof window !== 'undefined') {
			localStorage.setItem('sensecanvas-alert-history', JSON.stringify(get(alertHistory)));
		}
	},
	...
	showNotification(condition: AlertCondition, value: number) {
		...
		const notification = new Notification(`SenseCanvas Alert: ${condition.name}`, {
