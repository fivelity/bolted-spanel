...
>
  ...
  {#if showFrame}
    <CosmicFrame 
      ...
      style="filter: {glowEffect ? `drop-shadow(0 0 20px ${statusColor as string}40)` : 'none'}"
    />
  {/if}
  ...
  <div class="absolute inset-4 flex items-center justify-center">
    ...
    >
      ...
      <path 
        d={valueArc}
        stroke={statusColor as string}
        stroke-width="8"
        fill="none"
        stroke-linecap="round"
        class="transition-all duration-800 ease-out"
        style="filter: {glowEffect ? `drop-shadow(0 0 8px ${statusColor as string})` : 'none'}"
      />
