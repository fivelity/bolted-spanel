...
export function injectCosmicTheme() {
  ...
  root.style.setProperty('--color-primary-foreground', cosmicColors.primaryForeground);
  root.style.setProperty('--color-accent', cosmicColors.accent);
  root.style.setProperty('--color-accent-foreground', cosmicColors.accentForeground);
...
function createPathData(show: boolean, strokeWidth: string, stroke: string, fill: string, path: string[][]) {
  return {
    show,
    style: {
      strokeWidth,
...
export const defaultFramePaths = {
  ...
  panel: [
    createPathData(true, "1", "var(--color-frame-5-stroke)", "var(--color-frame-5-fill)", [
      ["M", "20", "20"],
      ["L", "80%", "20"], 
      ["L", "100% - 20", "35"],
      ["L", "100% - 20", "80%"],
